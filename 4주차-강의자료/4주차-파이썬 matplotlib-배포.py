...
# 선그래프, 마커모양은 'O',선스타일은 'dashed',  선색은 '파랑'
...
#그래프 색상 지정
# 기본색상으로 지정하기 --> Hexa코드 or CSS컬러 사용가능
...
# 가공된 데이터 신규 파일로 저장하기
file = 'c:/python/master/data/weather_data2.csv'    
...
# ### [실습] : 2중 축 그래프 그리기 :  평균상대습도 & 월합강수량 센터수

# In[41]:


# 2.'제주'지점의 평균상대습도 & 일최다강수량 센터수
data = df.groupby(['지점명','년도'])['평균상대습도'].agg(**{'월합강수량':'mean'}).reset_index()
data = data.query(" 지점명 == '제주' ")
X  = data['년도']
Y1 = data['평균상대습도']
